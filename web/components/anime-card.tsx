...
import { getStatusLabel, getConfidenceLabel } from "@/lib/utils"
...
export function AnimeCard({ mapping, index = 0 }: AnimeCardProps) {
  // 使用 React Query 钩子获取 AniList 数据
        ...
        <div className="bg-[#111] border border-[#222] rounded-lg overflow-hidden h-full flex flex-col">
          ...
          <div className="p-3 flex-1 flex flex-col">
            ...
            <div className="flex gap-1 mb-3">
              <div className="h-5 w-20 bg-[#222] rounded-md animate-pulse" />
              <div className="h-5 w-20 bg-[#222] rounded-md animate-pulse" />
            </div>
          </div>
        </div>
  ...
  // 从 AniList API 响应中提取数据
  const title = anilistData.title?.english || anilistData.title?.romaji || "Unknown Title"
  ...
  const type = anilistData.format || "TV"

  // 确保使用高质量的large封面图片
  const coverImage = anilistData.coverImage?.large || ""

  // 获取置信度信息
  const confidenceInfo = getConfidenceLabel(mapping.match_count)
    ...
    >
      <Link href={`/anime/${mapping.anilist_id}`} prefetch={true} scroll={false}>
        ...
        >
            ...
            >
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold
                  ${mapping.match_count >= 5 ? "bg-green-600" : mapping.match_count >= 3 ? "bg-yellow-600" : "bg-red-600"}`}
              >
                {mapping.match_count}
              </div>
            </motion.div>
          ...
          {/* Card Content - 移除嵌套的motion组件及variants，简化DOM结构 */}
          <div className="p-3 flex-1 flex flex-col">
            <h3 className="font-bold text-sm mb-1 line-clamp-1">
              {title}
            </h3>

            <p className="text-xs text-[#777] mb-2 line-clamp-1">
              {originalTitle}
            </p>
